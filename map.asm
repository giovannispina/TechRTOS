...
;map.c,28 :: 		int16 default_hash(void* ptr, int16 size)
;map.c,30 :: 		return ((int16)ptr % size);
...
;map.c,33 :: 		phashmap_t map_create(uint16 size, int16 (*hash)(void*, int16), int16 (*cmp)(void*, void*))
;map.c,37 :: 		if (!cmp) return NULL;
...
L_map_create0:
;map.c,39 :: 		hm = (phashmap_t) Malloc(sizeof(hashmap_t));
	MOVLW       14
...
;map.c,41 :: 		if (!hm) return NULL;
	...
	GOTO        L_end_map_create
L_map_create1:
;map.c,43 :: 		hm->buckets = Malloc(sizeof(plist_t)*size);
	...
	MOVF        R1, 0 
	MOVWF       POSTINC1+0 
;map.c,44 :: 		memset(hm->buckets, 0, sizeof(plist_t)*size);
...
;map.c,55 :: 		phashnode_t map_find(phashmap_t hm, void* key)
;map.c,60 :: 		index = hm->hash(key, hm->size);
	MOVLW       6
...
;map.c,62 :: 		if (!hm->buckets[index]) return NULL;
	...
	GOTO        L_end_map_find
L_map_find3:
;map.c,64 :: 		for (node = hm->buckets[index]->first; node; node = node->next) {
	MOVFF       FARG_map_find_hm+0, FSR0L+0
	MOVFF       FARG_map_find_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
...
L_map_find4:
	...
	BTFSC       STATUS+0, 2 
	GOTO        L_map_find5
;map.c,65 :: 		phashnode_t item = (phashnode_t)node;
	MOVF        map_find_node_L0+0, 0 
	MOVWF       map_find_item_L1+0 
	MOVF        map_find_node_L0+1, 0 
	MOVWF       map_find_item_L1+1 
;map.c,67 :: 		if (!hm->cmp(node->pdata, key))
	MOVLW       10
	ADDWF       FARG_map_find_hm+0, 0 
	MOVWF       FSR0L+0 
	MOVLW       0
...
;map.c,74 :: 		void map_insert(phashmap_t hm, void* key, void* value)
;map.c,80 :: 		index = hm->hash(key, hm->size);
	MOVLW       6
...
;map.c,82 :: 		if (!hm->buckets[index]) {
	...
	BTFSS       STATUS+0, 2 
	GOTO        L_map_insert8
;map.c,83 :: 		hm->buckets[index] = list_new();
	MOVFF       FARG_map_insert_hm+0, FSR0L+0
	MOVFF       FARG_map_insert_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
	...
	MOVF        R1, 0 
	MOVWF       POSTINC1+0 
;map.c,84 :: 		hm->buckets[index]->node_type = sizeof(hashnode_t);
	MOVFF       FARG_map_insert_hm+0, FSR0L+0
	MOVFF       FARG_map_insert_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
...
;map.c,85 :: 		} else {
	GOTO        L_map_insert9
L_map_insert8:
;map.c,86 :: 		for (node = hm->buckets[index]->first; node; node = node->next) {
	MOVFF       FARG_map_insert_hm+0, FSR0L+0
	MOVFF       FARG_map_insert_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
...
;map.c,94 :: 		}
L_map_insert9:
;map.c,96 :: 		item = (phashnode_t)list_insert(key, item);
	MOVF        FARG_map_insert_key+0, 0 
...
;map.c,100 :: 		void map_foreach(phashmap_t hm, void (*func)(void*, void*))
;map.c,105 :: 		for (i = 0; i < hm->size; i++) {
...
;map.c,106 :: 		if (!hm->buckets[i]) continue;
...
L_map_foreach17:
;map.c,108 :: 		for (node = hm->buckets[i]->first; node; node = node->next) {
	MOVFF       FARG_map_foreach_hm+0, FSR0L+0
	MOVFF       FARG_map_foreach_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
...
;map.c,110 :: 		func(node->pdata, item->value);
	MOVLW       4
...
;map.c,115 :: 		void map_erase(phashmap_t hm, pnode_t node)
;map.c,121 :: 		index = hm->hash(node->pdata, hm->size);
	MOVLW       6
...
;map.c,123 :: 		if (!hm->buckets[index]) return;
...
L_map_erase21:
;map.c,124 :: 		list_erase(hm->buckets[index], node);
	MOVFF       FARG_map_erase_hm+0, FSR0L+0
	MOVFF       FARG_map_erase_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
...
;map.c,127 :: 		void map_destroy(phashmap_t hm)
;map.c,132 :: 		if (!hm) return;
...
;map.c,136 :: 		list_destroy(hm->buckets[i]);
	MOVFF       FARG_map_destroy_hm+0, FSR0L+0
	MOVFF       FARG_map_destroy_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
	MOVWF       R3 
...
L_map_destroy24:
;map.c,139 :: 		FreeMem((void*)hm->buckets, sizeof(plist_t)*hm->size);
	MOVFF       FARG_map_destroy_hm+0, FSR0L+0
	MOVFF       FARG_map_destroy_hm+1, FSR0H+0
	MOVF        POSTINC0+0, 0 
